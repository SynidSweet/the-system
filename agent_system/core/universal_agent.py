...
class UniversalAgent:
    ...
    async def execute(self) -> AgentExecutionResult:
        ...
        try:
            ...
            if not safety_check["allowed"]:
                return await self._complete_with_error(safety_check["reason"])
    ...
    async def _execute_conversation(self) -> str:
        ...
        ai_provider = await ai_model_manager.get_provider(
            self.execution_context.agent.model_config
        )
        ...
        max_iterations = self.execution_context.agent.constraints.max_tool_calls
        iteration = 0
...
SAFETY CONSTRAINTS:
- Maximum tool calls: {self.execution_context.agent.constraints.max_tool_calls}
- Maximum execution time: {self.execution_context.agent.constraints.max_execution_time} seconds
- Recursion depth: {self.execution_context.recursion_depth} (max: {self.execution_context.agent.constraints.max_recursion_depth})
- Memory limit: {self.execution_context.agent.constraints.memory_limit_mb} MB
...
Complete your assigned task efficiently and precisely."""
    ...
    async def _execute_tool_with_context(self, tool_call: MCPToolCall) -> MCPToolResult:
        ...
        if tool_call.tool_name in ["start_subtask", "break_down_task", "end_task"]:
    ...
    async def _check_safety_constraints(self) -> Dict[str, Any]:
        """Check if agent execution is within safety constraints"""
        
        # Check recursion depth
        if self.execution_context.recursion_depth > self.execution_context.agent.constraints.max_recursion_depth:
            return {
                "allowed": False,
                "reason": f"Maximum recursion depth exceeded: {self.execution_context.recursion_depth}"
            }
        
        # Check if task is already completed
        if self.execution_context.task.status in [TaskStatus.COMPLETE, TaskStatus.FAILED]:
            return {
                "allowed": False,
                "reason": f"Task already completed with status: {self.execution_context.task.status}"
            }
