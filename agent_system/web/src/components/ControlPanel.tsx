...
import { ControlPanelProps, SystemConfig } from '../types';
...
const ControlPanel = ({ 
  ...
  const handleMaxTasksChange = useCallback((value: string) => {
  ...
  const handleThreadStepMode = useCallback((enabled: boolean) => {
    ...
    setLocalConfig(newConfig);
    onUpdateConfig(newConfig);
  }, [localConfig, selectedThread, onUpdateConfig]);
